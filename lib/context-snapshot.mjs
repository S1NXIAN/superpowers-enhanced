...
export function buildSnapshot(projectRoot) {
  ...
  try {
    ...
    let changedFiles = [];
    try {
      const commitCount = parseInt(execSync('git rev-list --count HEAD', { cwd: projectRoot, stdio: 'pipe' }).toString().trim(), 10);
      if (commitCount > 1) {
        ...
        const changedFilesRaw = execSync('git ls-tree -r --name-only HEAD', {
          cwd: projectRoot,
          stdio: 'pipe',
        }).toString().trim();
        changedFiles = changedFilesRaw ? changedFilesRaw.split('\n').filter(Boolean) : [];
