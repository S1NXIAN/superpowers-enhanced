...
export function truncateMap(content, lineLimit) {
  ...
  const contextNote = '*(Full map: zeus/memory/project-map.md — truncated from ' + lines.length + ' to ' + kept.length + ' lines)*';
  const footer = '\n\n' + contextNote;
  return kept.join('\n') + footer;
}
...
function printTree(dir, prefix, lines, root, depth = 0) {
  if (depth >= 3) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (stat.isDirectory()) {
      ...
      printTree(fullPath, nextPrefix, lines, root, depth + 1);
    } else {
      lines.push(prefix + connector + entry);
    }
  }
